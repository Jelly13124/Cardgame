...
const OUTPUT_PATH = "res://tmp/gunslinger-card-ui-runtime.png"
...
func _capture() -> void:
	...
	for index in range(CARD_IDS.size()):
		...
		var row := index / 4
		var column := index % 4
		var row_offset := 225.0 if row == 2 else 0.0
	...
	if error != OK:
		push_error("Could not save Gunslinger card capture: %s" % error)
		get_tree().quit(1)
		return
	get_tree().quit(0)
