...
const OUTPUT_PATH = "res://tmp/card-ui-runtime-imagegen-v3.png"
const CARD_IDS: Array[String] = ["weak_strike", "strike", "defend", "focus"]
...
func _capture() -> void:
	...
	veil.color = Color(0.03, 0.075, 0.08, 0.30)
	...
	for index in range(CARD_IDS.size()):
		...
		if card == null:
			continue
		card.position = Vector2(START_X + GAP * index, 305.0)
	...
	if error != OK:
		push_error("Could not save card UI visual capture: %s" % error)
		get_tree().quit(1)
		return
	get_tree().quit(0)
