...
*   **Character Window**: `run_system/ui/window/character_window.gd` — draggable window (press **i** anywhere, or the top-bar Character button): hero picker + 5 equip slots + stash/backpack grid + sets/relics/stats. Modes: `base` (stash + next-run loadout + carry marks), `map` (editable), `battle` (read-only). Replaces the old fullscreen `equipment_panel.gd`. Window infra: `window/draggable_window.gd` + `window/window_layer.gd` (z-order, ESC closes topmost).
...
*   **Boot Scene**: `run_system/ui/home_base_scene.{gd,tscn}` — global currency top bar (`window/currency_top_bar.gd`, Core/Caps/Scrap + Character button) + 4 building tiles laid out 2-left / 2-right with a centre START "door". Forge opens draggable windows; Clinic/Market/Outpost open fullscreen screens. Press **i** for the CharacterWindow.
*   **Building Screens**: `run_system/ui/buildings/{clinic,market,outpost}_screen.gd` (subclass `building_screen_base.gd`) — **fullscreen, services-only** pages; `home_base_scene` convention-loads `<id>_screen.gd`. **Unlock/upgrade lives on the home-base overview** (button under each building's floating label → confirm popup → `MetaProgress.unlock/upgrade_building`). Per-building: market = tool shelf + refresh (T1) / equipment shelf (T2) / resource conversion (T3); outpost builds its Core-upgrade rows inline. **Forge is a draggable window** (`window/forge_window.gd`, 4 tabs: craft/dismantle/reforge/curse; opens beside the CharacterWindow for cross-window gear drags). The former warehouse screen was removed (loadout → CharacterWindow base mode).
