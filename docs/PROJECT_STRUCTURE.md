...
*   **Boot Scene**: `run_system/ui/home_base_scene.{gd,tscn}` — 4 building tiles in a centered row (x 195/585/975/1365), giant START button bottom-centre + difficulty button above it (A0-A5 picker popup), bottom currency bar (`window/currency_top_bar.gd`, Core/Caps/Scrap), right-edge image buttons: Stash (warehouse art; opens StashWindow + CharacterWindow side by side) and Character (hero headshot; toggles CharacterWindow). Forge opens draggable windows; Clinic/Market/Outpost open fullscreen screens. Press **i** for the CharacterWindow.
