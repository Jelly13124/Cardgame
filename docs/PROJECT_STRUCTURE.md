...
*   **Boot Scene**: `run_system/ui/home_base_scene.{gd,tscn}` — 4 building tiles in a centered row (x 195/585/975/1365) + a unified **bottom HUD bar** (`window/currency_top_bar.gd`, anchored full-width bottom, exposes left/center/right boxes): currency chips left, giant START + difficulty button (A0-A5 picker popup) center, Stash + Character image buttons right (Stash opens StashWindow + CharacterWindow side by side; Character toggles CharacterWindow). Theme hooks in `wasteland_theme.gd` (`ui_bottom_bar` / `ui_button_brass` / `ui_button_accent` + window/slot styles) auto-skin from `run_system/assets/images/ui_kit/` when the Codex kit lands (spec: `docs/asset-spec-ui-kit.md`, rev2 pending). Forge opens draggable windows; Clinic/Market/Outpost open fullscreen screens. Press **i** for the CharacterWindow.
