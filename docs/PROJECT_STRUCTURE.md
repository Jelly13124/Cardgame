...
*   **Building Screens**: `run_system/ui/buildings/{clinic,market,outpost}_screen.gd` (subclass `building_screen_base.gd`) — **fullscreen, services-only** pages; `home_base_scene` convention-loads `<id>_screen.gd`. **Unlock/upgrade lives on the home-base overview** (button under each building's floating label → confirm popup → `MetaProgress.unlock/upgrade_building`). Per-building: market = **daily bounty shelf** (T1, 1 free claim + 2 Caps contracts, date-seeded reroll) + tool shelf + refresh (T1) / equipment shelf (T2) / resource conversion (T3); outpost builds its Core-upgrade rows inline. **Forge is a draggable window** (`window/forge_window.gd`, 4 tabs: craft/dismantle/reforge/curse; opens beside the CharacterWindow for cross-window gear drags). The former warehouse screen was removed (loadout → CharacterWindow base mode).
