...
| Shared theme | `run_system/ui/theme/wasteland_theme.gd` (legacy filename; palette must follow Rick and Morty-style Sci-Fi Cartoon Wasteland) |
...
- The shared theme palette lives in `run_system/ui/theme/wasteland_theme.gd` - read its constants before defining new colors locally, but keep new colors aligned with the Rick and Morty-style Sci-Fi Cartoon Wasteland direction.
