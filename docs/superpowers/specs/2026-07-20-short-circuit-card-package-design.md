...
- **Detonate** removes all Short Circuit from the target and deals damage equal to the removed stacks.
- Detonation is status damage: it is not increased by Strength and does not trigger Thorns.
- **Critical Discharge** lets player-caused detonations use Bill's normal Luck-based Crit chance.
...
| `arc_flash` | Arc Flash | 1 / common | Deal 2 to all; detonate Short Circuit on all enemies. Upgrade damage to 4. |
...
| `limit_break` | Overload | 1 / rare | Double target Short Circuit, then detonate it. Exhaust. Upgrade cost to 0. |
| `hemorrhage` | Critical Discharge | 2 / rare | This combat, player Short Circuit detonations can Crit. Upgrade cost to 1. |
...
- Card descriptions must explicitly say whether Short Circuit is applied, detonated, doubled, or removed for Block.
